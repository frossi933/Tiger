...
struct
   ...
   (*val _ = print("Size:"^makestring arr^"\n")*)
			val _ = if (idx>=siz orelse idx<0) then raise Fail("Ãndice fuera de rango"^" siz:"^Int.toString siz^" idx:"^Int.toString idx^"\n") else ()
		in
			0
		end
		...
		and evalFun(f, args) =
			let
				...
				val _ = if (List.length(ffrac)>1) then raise Fail ("Funcion repetida: "^f^"\n") 
				        else if (List.length(ffrac)=0) then raise Fail ("No se encuentra la funcion: "^f^"\n") else ()
				...
				(* Poner argumentos donde la funciÃ³n los espera *)
            ...
            | _ => raise Fail "Esto no es una direcciÃ³n de memoria ni registro") formalsValues
