	...
	let fun error(s, p) = raise Fail ("Error -- línea "^Int.toString(p)^": "^s^"\n")
		...
		| trexp(OpExp({left, oper, right}, nl)) = 
			...
			in
				if tiposIguales tyl tyr then
					case oper of
						PlusOp => if tipoReal tyl=TInt then 
							{exp=binOpIntExp {left=expl, oper=oper, right=expr},
							ty=TInt} else error("Error de tipos", nl)
						| MinusOp => if tipoReal tyl=TInt then
							{exp=binOpIntExp {left=expl, oper=oper, right=expr},
							ty=TInt} else error("Error de tipos", nl)
						| TimesOp => if tipoReal tyl=TInt then 
							{exp=binOpIntExp {left=expl, oper=oper, right=expr},
							ty=TInt} else error("Error de tipos", nl)
						| DivideOp => if tipoReal tyl=TInt then 
							{exp=binOpIntExp {left=expl, oper=oper, right=expr},
							ty=TInt} else error("Error de tipos", nl)
						| LtOp => if tipoReal tyl=TInt orelse tipoReal tyl=TString then
							{exp=if tipoReal tyl=TInt then 
								binOpIntRelExp {left=expl,oper=oper,right=expr} 
								else binOpStrExp{left=expl,oper=oper,right=expr},
								ty=TInt} 
		...
		| trexp(RecordExp({fields, typ}, nl)) =
			let
				...
				(*Verificar q c/campo esté en orden y tenga una expres del tipo q corresponde *)
				fun verificar _ [] [] = []
				...
				val lf = verificar 0 cs tfields (*no siguen el mismo orden, habria queordenarlos*)
			in
